...
function User:get_identifiers(source)
    local target_source = source or self.source
    local ids = {}
    local raw_ids = GetPlayerIdentifiers(target_source)
    if raw_ids then
...
function User:set_username(username)
    ...
    if _db.value_exists("users", "username", username) then
