...
local _db = require("core.server.modules.database")
...
function UserRegistry:request_connection(source, name, deferrals)
    ...
    deferrals.defer()
    update_deferral(deferrals, "core.server.user.registry.checking")

    local result = self:exists(ids.license)
    local user_data = result and result[1]

    if not user_data then
        update_deferral(deferrals, "core.server.user.registry.creating")
    ...
    update_deferral(deferrals, "core.server.user.registry.checking_bans")
