...
export class InventoryGrid {
    ...
    _init_drag_and_drop() {
        ...
        const with_layers_passthrough = (x, y) => {
            const layers = document.querySelectorAll(".grid_items, .grid_item");
        ...
        if (!InventoryGrid._global_bound) {
            ...
            $(document).on("pointerup.griddrag_global pointercancel.griddrag_global", async (e) => {
                ...
                console.log("[griddrag] drop fired. cell:", cell);
                ...
                const slot = get_slot_at(e.clientX, e.clientY);
                console.log("[griddrag] no cell, checking slot:", slot, "on_drop_to_slot exists:", typeof d.on_drop_to_slot);

                if (slot && typeof d.on_drop_to_slot === "function") {
                    console.log("[griddrag] calling on_drop_to_slot with", d.item_id, d.from_col, d.from_row, d.from_section, slot.slot_num, slot.group_id, slot.section);
