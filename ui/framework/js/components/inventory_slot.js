...
export class InventorySlot {
        ...
        const groups_html = this.groups.map((group, group_index) => {
            ...
            if (layout_type === "positioned" && group.slots) {
                ...
                slots_html = slots.map((item, i) => {
                    ...
                    if (show_slot_numbers !== null && typeof show_slot_numbers === 'object' && !Array.isArray(show_slot_numbers)) {
                        if (show_slot_numbers[i + 1] !== undefined) {
                            slot_number = show_slot_numbers[i + 1];
                        }
                    } else if (Array.isArray(show_slot_numbers)) {
    ...
    create_slot(item, index, group_id, slot_number = null, slot_id = null, size = null) {
        ...
        const empty_class = item.empty ? "slot_out_of_stock" : "";
        ...
        return `
            <div class="body_slot ${rarity_class} slot_occupied ${empty_class}"
                ${size_style}
                data-slot-index="${index}"
                data-slot-num="${slot_num}"
                data-group-id="${group_id}"
    ...
    init_drag_and_drop(container) {
        ...
        if (!InventorySlot._global_bound) {
            ...
            $(document).on("pointerup.invdrag_global pointercancel.invdrag_global", async (e) => {
                ...
                const under = document.elementFromPoint(e.clientX, e.clientY);
                const to_el = under ? under.closest(".body_slot") || under.closest(".positioned_slot_wrapper")?.querySelector(".body_slot") : null;
                ...
                if (d.from_section === "hotbar" && typeof d.on_drop_to_grid === "function") {
                    await d.on_drop_to_grid(String(d.from_slot), String(d.from_group), String(d.from_section), null, null, null, d.dataset);
                }
